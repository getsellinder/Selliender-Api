...
// export const getAllChapterAdmin = async (req, res) => {
//   try {
...
//         select: "name genreName tax active active ",
//       })
//       .limit(PAGE_SIZE)
//       .skip(PAGE_SIZE * page)
//       // .sort("name")
...
export const getAllChapterAdmin = async (req, res) => {
  try {
    ...
    if (req.query?.genre) obj.genre = req.query.genre;
    if (req.query?.subject) obj.subject = req.query.subject;
    // if (req.query?.FeatureProduct)
    //   obj.featured_Product = req.query.FeatureProduct;
    const total = await Chapter.countDocuments(obj);
    const chapterdata = await Chapter.find({ ...obj, isDeleted: false })
      // .populate({
      //   path: "genre addedBy master_GST variants.gst_Id subject",
      //   select: "name genreName tax active active subjectName",
      // })
      .limit(PAGE_SIZE)
      .skip(PAGE_SIZE * page)
      // .sort("name")
    ...
    for (let ch of chapterdata) {
      const genrname = await GenreModel.findOne({ _id: ch.genre });
      const subjectname = await SubjectModel.findOne({ _id: ch.subject });
      if (!genrname) {
        console.warn(`Genre not found for ID: ${ch.genre}`);
      }
      if (!subjectname) {
        console.warn(`Subject not found for ID: ${ch.subject}`);
      }

      chapter.push({
        // chapter: ch,
        ...ch._doc,
        genrname: genrname.genreName,
        subjectname: subjectname.subjectName,
      });
      // console.log("genrname", genrname);
      // console.log("subjectname", subjectname.subjectName);
...
export const getAllDeletedChapters = async (req, res) => {
  try {
    const PAGE_SIZE = parseInt(req.query?.show || "10");
    const page = parseInt(req.query?.page - 1 || "0");
    let obj = {};
    ...
    //   obj.featured_Product = req.query.FeatureProduct;
    const total = await Chapter.countDocuments(obj);
    const chapterdata = await Chapter.find({ ...obj, isDeleted: true })
      // .populate({
      //   path: "genre addedBy master_GST variants.gst_Id subject",
      //   select: "name genreName tax active active subjectName",
      // })
...
export const getAllChaptersDevicesFirst = async (req, res) => {
  try {
    ...
    const genreName = "Jazzii";
    // Find the category object by name first
    const genre = await GenreModel.findOne({ genreName });
    if (!genre) {
      throw new Error("Genre not found");
    }
    // products with device category
    const deviceProducts = await Chapter.find({
      genre: genre._id,
    }).populate("genre");

    // all products
    const allProducts = await Chapter.find()
      .populate({
        path: "genre gst addedBy",
        select: "name genreName tax",
      })
      .sort({
        createdAt: -1,
      });
    ...
    const filteredProducts = allProducts.filter((ele) => {
      return ele.genre?.genreName !== genreName;
    });
...
export const getOneChapter = async (req, res) => {
  try {
    const data = await Chapter.findById(req.params.id).populate([
      {
        path: "genre",
        select: "genreName", // or whatever field name exists in GenreModel
      },
      {
        path: "subject",
        select: "subjectName", // or whatever field name exists in SubjectModel
      },
      {
        path: "addedBy",
