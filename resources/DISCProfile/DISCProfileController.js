...
export const analyzeDISCProfile = async (req, res) => {
  try {
    const { userId, linkedinContentId, linkedinPostId, productDescription } =
      req.body;
    ...
    if (!profileData) {
      return res.status(404).json({
        message:
          "LinkedIn profile data not found. Please upload LinkedIn data first.",
      });
    }
    ...
    console.log(
      `Analyzing profile for ${profileData.name} with ${postsData.length} posts`
    );
    ...
    if (postsData.length > 0)
      dataSources.push(`posts_${postsData.length}_items`);
    ...
    if (name) {
      discProfiles = discProfiles.filter((profile) => {
        if (profile.linkedinContentId && profile.linkedinContentId.name) {
          return profile.linkedinContentId.name
            .toLowerCase()
...
export const reanalyzeDISCProfile = async (req, res) => {
  try {
    ...
    const profileData = await LinkedinContent.findById(
      existingProfile.linkedinContentId
    );
    let postsData = [];

    if (existingProfile.linkedinPostId) {
      const postsDoc = await LinkedinPost.findById(
        existingProfile.linkedinPostId
      );
      if (postsDoc && postsDoc.posts) {
        postsData = postsDoc.posts;
      }
    ...
    const newProductDescription =
      productDescription || existingProfile.productDescription;
